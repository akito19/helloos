...
void sheet_updown(struct SHEET *sht, int height)
{
    ...
    if (old > height) { // 以前よりも低くなる
        if (height >= 0) {
            ...
            ctl->top--; // 表示中のシートが1つ減るので，一番上の高さが変わる
        ...
        if (old >= 0) {
            ...
            ctl->sheets[height] = sht;
            ctl->top++; // 表示中のシートが1つ増えるので，一番上の高さが増える
        }
        // 新しいシートの情報に沿って画面を描き直す
...
{
    if (sht->height >= 0) { // もしも表示中なら，新しいシートの情報に沿って画面を描き直す
...
void sheet_slide(struct SHEET *sht, int vx0, int vy0)
{
    ...
    sht->vx0 = vx0;
    sht->vy0 = vy0;
    if (sht->height >= 0) { // もし表示中なら新しいシートの情報に沿って画面を描き直す
...
void sheet_refreshmap(struct SHTCTL *ctl, int vx0, int vy0, int vx1, int vy1, int h0)
{
    ...
    for (h = h0; h <= ctl->top; h++) {
        sht = ctl->sheets[h];
        sid = sht - ctl->sheets0; // sheetの番地との差を id として利用
        buf = sht->buf;
        bx0 = vx0 - sht->vx0;
        by0 = vy0 - sht->vy0;
        bx1 = vx1 - sht->vx0;
